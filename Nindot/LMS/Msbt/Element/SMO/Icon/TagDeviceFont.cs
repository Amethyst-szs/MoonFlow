...
public class MsbtTagElementDeviceFont : MsbtTagElement
{
    ...
    internal override void InitTag(ref int pointer, byte[] buffer, ushort dataSize)
    {
        ...
        if (fntIdx != (ushort)FontIndex)
            Console.WriteLine("DeviceFont tag has non-0 value in first data place, setting to 0");
        ...
        {
            Console.Error.WriteLine("DeviceFont tag has mismatch between IconType and char data buffer, setting to default icon");
